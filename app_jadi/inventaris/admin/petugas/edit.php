<?php  
	...
	if ($_POST) {
		...
		if ($simpan) {
			$_SESSION['status'] = 'Data Petugas berhasil Diubah!';
	...
	<main>
		...
		<form method="post" action="">
			<div class="input-wrapper">
				...
				<input type="text" value="<?= $data['nama'] ?>" name="nama_petugas" required>
			</div>
			...
			<div class="input-wrapper">
				...
				<input type="text" value="<?= $data['password'] ?>" name="password" required>
			</div>
				...
				<select name="level">
					<?php  
						while ($data_level = mysqli_fetch_array($level)) { ?>
							<option <?= $data['id_level'] == $data_level['id_level'] ? 'selected' :''?> value="<?= $data_level['id_level'] ?>"> 
